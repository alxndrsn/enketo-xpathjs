/* global define, describe, xdescribe, require, it, xit, before, after, beforeEach, afterEach, expect, Blob, documentEvaluate, window, loadXMLFile */
"use strict";

var doc, win, docEvaluate;

win = window;
// replace native XPath evaluator
win.XPathJS.createDomLevel3XPathBindings({
    'case-sensitive': true
});
docEvaluate = win.document.evaluate;

describe('custom(ized) functions', function() {
    ...
    it('position(node)', function() {
        ...
        for (i = 0; i < input.length; i++) {
            ...
            expect(input[i][2]).to.equal(result.numberValue);
        }
    });
});
